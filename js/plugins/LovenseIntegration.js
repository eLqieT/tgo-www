...
(function() {
	...
	if(typeof PluginManager.parameters('LovenseIntegration')["Option Menu Shop Text"] !== "undefined") {
		...
		SR.LovenseIntegration.optionStoreText = "Shop at Lovense";
	}
	...
	Window_Options.prototype.makeCommandList = function() {
		...
		this.addCommand("Lovense Intensity", 'lovenseIntensity');
...
//-----------------------------------------------------------------------------
	...
	LovenseAddParent.prototype.createHelpWindowConnecting = function() {
        this._helpWindow = new Window_Help(1);
        this._helpWindow.setText("Checking internet connection.");
        this.addWindow(this._helpWindow);
    };

	LovenseAddParent.prototype.createHelpWindowNoConnection = function() {
        this._helpWindow = new Window_Help(1);
        this._helpWindow.setText("No internet connection available.");
        this.addWindow(this._helpWindow);
    };

    LovenseAddParent.prototype.createHelpWindow = function() {
        this._helpWindow = new Window_Help(3);
        this._helpWindow.setText("In the Lovense Remote App click the plus(+) in the upper-right. Then select \"Scan QR\".\n\nAfter scanning, the device list will appear shortly.");
        this.addWindow(this._helpWindow);
    };
			...
			if (xhr.status === 200) {
				if(xhr.responseText.indexOf(',') != -1) {
					...
					function setDynamicInterval() {
						// If there's an interval already set, clear it
						if (window.webInterval) {
							clearInterval(window.webInterval);
						}
						window.webInterval = setInterval(function() {
							var xhr2 = new XMLHttpRequest();
							xhr2.open("GET", "http://www.nlt-media.net/lovenseConnect/check.php?requestid=" + self.LAP.requestId + "&uid=" + self.LAP.uid.toString() + "&utoken=" +  self.LAP.utoken, true);
							xhr2.setRequestHeader("Cache-Control", "no-cache");
							xhr2.onload = function (e) {
							  if (xhr2.readyState === 4) {
								if (xhr2.status === 200) {
									if(xhr2.responseText != "NO RESPONSE YET" && xhr2.responseText != "too many requests") {
										...
										if(xhr2.responseText == "too many requests") {
											waitTime = 10000;
											setDynamicInterval();
										}
									}
    ...
    Window_DeviceList.prototype.drawItem = function(index) {
		...
		if(id == this.numOfDevices && lovense.connectionType() != 'PC') {
			if(this.numOfDevices > 1) {
				this.drawText("Re-link Account or Connect New Account", rect.x, rect.y+(rect.height/2)-15, rect.width, "center");
			} else {
				this.drawText("Connect Account", rect.x, rect.y+50, rect.width, "center");
			}
		} else {
			if(this.secondDevice=="Warning" && toys.length == 0) {
				this.drawText("Unable to find toys.", rect.x, rect.y+(rect.height/2)-30, rect.width, "center");
				this.drawText("Ensure app is active and toys are connected.", rect.x, rect.y+(rect.height/2), rect.width, "center");
				this.drawText("Then click here to re-sync.", rect.x, rect.y+(rect.height/2)+30, rect.width, "center");
			} else {
				...
				this.drawText("Connection Type: " + lovense.connectionType(), rect.x-160, rect.y+60, rect.width, 'right');
				if(lovense.connectionType() != "Web") {
					if(toys[index].status == 0) {
						this.drawText("Not connected", rect.x+260, rect.y+60, rect.width);
					} else {
						this.drawText("Connected", rect.x+260, rect.y+60, rect.width);
					}
				}
				var fs = require ("fs");
...
;(function() {
	...
	lovenseCommand = function(intensity) {
		...
		var standardCommand = '{"command":"Function","action":"All:';
